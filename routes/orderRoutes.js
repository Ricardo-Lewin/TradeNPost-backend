...
router.post('/', async(req, res)=> {
  ...
  try {
    ...
    const notification = {status: 'unread', message: `New order from ${user.name}`, time: new Date().toLocaleString('en-US')};
...
router.patch('/:id/mark-shipped', async(req, res)=> {
  ...
  try {
    ...
    const notification = {status: 'unread', message: `Order ${id} shipped with success`, time: new Date().toLocaleString('en-US')};
